...
/**
 * An {@link EventStudio} is a thread-safe central place allowing broadcast of events to {@link Listener}s to registered on a Station. Stations are created
 * internally as soon as a listener is added but it's up to the user to clear a station when it's not needed anymore. {@link Listener}s can be added multiple
...
public interface EventStudio {
    ...
    /**
     * Adds the given {@link Listener} to the given station using the given priority (low values mean higher priority), strength and once flag.
     *
     * @param once the listener will be executed once and then removed
     */
    <T> void add(Listener<T> listener, String station, int priority, ReferenceStrength strength, boolean once);

    /**
     * Adds the given {@link Listener}, listening for the given event class, to the given station using default priority(0) ad default strength
     * {@link ReferenceStrength#STRONG}. This adds method is useful when a listener can listen for a hierarchy of events.
     *
    ...
    /**
     * Adds the given {@link Listener}, listening for the given event class, to the given station using the given priority (low values mean higher priority) and
     * strength. This adds method is useful when a listener can listen for a hierarchy of events:
     *
     * <pre>
     * {@code
    ...
    /**
     * Adds the given {@link Listener}, listening for the given event class, to the given station. This adds method is useful when a listener can listen for a
     * hierarchy of events.
     *
     * @param once the listener will be executed once and then removed
     * @see EventStudio#add(Class, Listener, String, int, ReferenceStrength)
     */
    <T> void add(Class<T> eventClass, Listener<T> listener, String station, int priority, ReferenceStrength strength, boolean once);
    ...
    /**
     * Sets a {@link Supervisor} for the given station. It will be notified of every event broadcasted to the station prior its delivery to the proper
     * {@link Listener}s allowing event inspection.
     * <p>
    ...
    /**
     * Clear the given station removing the whole station from the {@link EventStudio} which means that {@link Listener}s and {@link Supervisor} will not be
     * notified anymore. A station with the same name can be recreated.
     */
