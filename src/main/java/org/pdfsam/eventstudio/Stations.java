...
class Stations {
     ...
     * @throws IllegalArgumentException if the station name is blank or null
     */
    Station getStation(String stationName) {
        requireNotBlank(stationName);
        return stations.computeIfAbsent(stationName, name -> {
    ...
    void clear(String stationName) {
        LOG.debug("Clearing station {}", stationName);
