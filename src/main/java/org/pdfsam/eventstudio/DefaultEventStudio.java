...
/**
 * Default implementation of {@link EventStudio}. It doesn't enforce a Singleton pattern, and it's up to the user to decide how to use it and how many
 * EventStudio the application needs. A singleton implementation with lazy initialization is provided with {@link org.pdfsam.eventstudio.StaticStudio} where the
 * typical usage is:
 *
 * <pre>
 * {@code
 ...
 * </pre>
 * <p>
 * <b>Hidden Station</b>: The hidden station is a pre-built station with <em>"hidden.station"</em> name that is used to hide the station abstraction. Helper
 * method are provided by {@link DefaultEventStudio} where the station name parameter is missing from the parameters list and the
 * {@link DefaultEventStudio#HIDDEN_STATION} is used, providing a more traditional event bus pub/sub pattern.
 * </p>
 *
...
public class DefaultEventStudio implements EventStudio {
    /**
     * A reserved station name that is used to hide the station abstraction. Using the provided helper methods the station concept remains totally hidden and
     * {@link EventStudio} can be used as a more traditional Event Bus with pub/sub pattern.
     */
    ...
    @Override
    public <T> void add(Listener<T> listener, String station, int priority, ReferenceStrength strength, boolean once) {
        stations.getStation(station).add(listener, priority, strength, once);
    }

    @Override
    public <T> void add(Listener<T> listener, String station, int priority, ReferenceStrength strength) {
        add(listener, station, priority, strength, false);
    }
    ...
    public <T> void add(Class<T> eventClass, Listener<T> listener, String station, int priority,
        ...
        add(eventClass, listener, station, priority, strength, false);
    }

    @Override
    public <T> void add(Class<T> eventClass, Listener<T> listener, String station, int priority,
            ReferenceStrength strength, boolean once) {
        stations.getStation(station).add(eventClass, listener, priority, strength, once);
    }
    ...
    /**
     * Adds a {@link Listener} (with the given priority and strength ) to the hidden station listening for the given event class, hiding the station
     * abstraction.
     *
